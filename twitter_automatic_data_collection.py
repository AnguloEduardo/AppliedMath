...
def start_extraction(search_date, destination_folder):
    ...
    language = 'en'
    search_terms = '#COVID19 OR #VaccineCOVID19 OR #NoVaccine OR #VaccineSideEffects'
    query = '{} place:{}'.format(search_terms, place_id)
    print(query)
    ...
    while True:
        try:
            ...
            print('PAUSED --> On ' + places[0].country_code + ' collecting ' + str(
    ...
    print('timestamp: ', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    print('For ' + places[0].country_code + ' collected ' + str(len(tweet_id)) + ' tweets')
    print()
    print()
    ...
    df.to_csv(destination_folder + places[0].country_code + '_' + str(search_date).replace('-', '_') + '.csv',
              index=False, encoding='utf_8_sig', sep='\t')
